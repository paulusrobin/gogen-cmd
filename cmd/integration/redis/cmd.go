...
func runner(cmd *cobra.Command, args []string) error {
	log.Printf("adding a redis integration")
	...
	if err := project.AddDependencies(parameter.AddDependency{
		...
		Dependency: parameter.Redis,
	}); err != nil {
		return err
	}

	log.Printf("succesfully add redis integration")
	return nil
}
...
func Cmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "redis",
		Short: "add redis dependency on gogen project",
