...
func generateObject(parameters parameter.ProjectConfigWithPackage) error {
	...
	fileOutput := path.Join(parameters.Path, "internal/pkg", packageFileName, "endpoint/root.go")
	if file.Exist(fileOutput) {
		return nil
	}

	return file.Generate(fileOutput, string(endpointObjectTemplate), map[string]interface{}{
...
func generateRoot(parameters parameter.ProjectConfigWithPackage) error {
	...
	fileOutput := path.Join(packagePath, "endpoint.go")

	fileNames, err := directory.FileNames(path.Join(packagePath, "endpoint"))
	if err != nil {
		return err
	}
	...
	return file.Generate(fileOutput, string(endpointRootTemplate), map[string]interface{}{
		"ProjectName":   parameters.Name,
		"ProjectModule": parameters.Module,
		"PackageName":   parameters.PackageName,
		"Endpoints":     endpoints,
